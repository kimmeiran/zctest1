  ...
  <div class="score-analysis-container">
    <div class="content-wrapper">
      ...
      <div class="page-header">
        <h1 class="main-title">考试成绩分析</h1>
        <p class="subtitle">当前考试的成绩统计分析</p>
        ...
        <div class="stat-card">
          <div class="stat-content">
            ...
            <div class="stat-info">
              ...
              <p class="stat-value">{{ passRateText }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="stat-card">
          <div class="stat-content">
            ...
            <div class="stat-info">
              ...
              <p class="stat-value">{{ jobTypeCountDisplay }}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table class="stats-table">
            <thead>
              <tr>
                ...
                <th>占总人数比例</th>
                <th class="th-center">人数排名</th>
                <th class="th-center">分数等级</th>
              </tr>
            </thead>
            <tbody>
...
import { Users, TrendingUp, Award, Building2, BarChart3, Search, ChevronDown, ChevronUp, PieChart, BarChart, CheckCircle } from 'lucide-vue-next'
...
// 接口数据：list 为原学生列表，汇总字段由接口直接返回
const totalParticipants = ref(0)
const rawStudentList = ref([])
const loading = ref(false)
// 接口返回的汇总（GET /api/teachExam/getAllStdentsScore 新格式）
...
const fetchAnalysis = async () => {
  ...
  loading.value = true
  try {
    const res = await getAllStudentsScore(props.examId, undefined)
    if (res.code === 200 && res.data) {
      const data = res.data
      rawStudentList.value = Array.isArray(data.list) ? data.list : (data.list != null ? [data.list] : [])
      totalParticipants.value = data.registrationCount ?? rawStudentList.value.length
      summaryStats.value = {
        registrationCount: data.registrationCount ?? 0,
        passCount: data.passCount ?? 0,
        passRate: data.passRate != null ? String(data.passRate) : '',
        jobTypeCount: data.jobTypeCount ?? 0,
        maxScore: data.maxScore ?? 0,
        minScore: data.minScore ?? 0,
        avgScore: data.avgScore != null ? String(data.avgScore) : ''
      }
    } else {
      rawStudentList.value = []
      totalParticipants.value = 0
      summaryStats.value = { registrationCount: 0, passCount: 0, passRate: '', jobTypeCount: 0, maxScore: 0, minScore: 0, avgScore: '' }
    }
  } catch (e) {
    console.error('获取考试成绩数据失败:', e)
    totalParticipants.value = 0
    rawStudentList.value = []
    summaryStats.value = { registrationCount: 0, passCount: 0, passRate: '', jobTypeCount: 0, maxScore: 0, minScore: 0, avgScore: '' }
  } finally {
...
// 全局统计：优先使用接口返回的汇总，否则由 list 计算
const averageScore = computed(() => {
  const apiVal = summaryStats.value.avgScore
  if (apiVal !== '' && apiVal !== undefined) return apiVal
...
const highestScore = computed(() => {
  const apiVal = summaryStats.value.maxScore
  if (apiVal !== undefined && apiVal !== null && apiVal !== '') return Number(apiVal)
  const allScores = rawStudentList.value.map(student => student.totalScore || 0)
  if (!allScores.length) return 0
  return Math.max(...allScores)
})

// 通过率：优先使用接口返回的 passRate
const passRateText = computed(() => {
  const apiRate = summaryStats.value.passRate
  if (apiRate !== '' && apiRate !== undefined) return apiRate.includes('%') ? apiRate : apiRate + '%'
  const total = totalParticipants.value
  if (!total) return '0%'
  const passed = rawStudentList.value.filter(s => (Number(s.totalScore) || 0) >= 60).length
  return Math.round((passed / total) * 100) + '%'
})

// 职业(工种)数：优先使用接口返回的 jobTypeCount
const jobTypeCountDisplay = computed(() => {
  const apiCount = summaryStats.value.jobTypeCount
  if (apiCount > 0 || summaryStats.value.registrationCount > 0) return apiCount
  return employeeCategories.value.length
})
...
.score-bar-green { background: linear-gradient(90deg, #10b981 0%, #34d399 100%); }
.score-bar-blue { background: linear-gradient(90deg, #3b82f6 0%, #60a5fa 100%); }
.score-bar-yellow { background: linear-gradient(90deg, #f59e0b 0%, #fbbf24 100%); }
.score-bar-orange { background: linear-gradient(90deg, #f97316 0%, #fb923c 100%); }
.score-bar-red { background: linear-gradient(90deg, #ef4444 0%, #f87171 100%); }
...
/* 人数排名、分数等级：表头与单元格内容居中对齐 */
.stats-table th.th-center {
  text-align: center;
}
...
.rank-cell .badge-cell-inner,
.grade-cell .badge-cell-inner {
  display: flex;
  justify-content: center;
  align-items: center;
}
...
.rank-high { background-color: #dcfce7; color: #166534; border-color: #86efac; }
.rank-good { background-color: #dbeafe; color: #1e40af; border-color: #93c5fd; }
.rank-medium { background-color: #fef3c7; color: #92400e; border-color: #fde68a; }
.rank-low { background-color: #ffedd5; color: #9a3412; border-color: #fdba74; }
.rank-very-low { background-color: #fee2e2; color: #991b1b; border-color: #fca5a5; }

.grade-excellent { background-color: #dcfce7; color: #166534; border-color: #86efac; }
.grade-good { background-color: #dbeafe; color: #1e40af; border-color: #93c5fd; }
.grade-medium { background-color: #fef3c7; color: #92400e; border-color: #fde68a; }
.grade-pass { background-color: #ffedd5; color: #9a3412; border-color: #fdba74; }
.grade-fail { background-color: #fee2e2; color: #991b1b; border-color: #fca5a5; }

    ...
    <div class="paper-management">
        ...
        <template v-if="!showPaperDetail">
            ...
            <div class="action-buttons">
                ...
                <button class="btn btn-dispatch" @click="showMultiDispatchModal"
                ...
                <div class="exam-info-header">
                    <div class="info-section">
                        <div class="info-item">
                            ...
                            <span class="info-value">{{ currentPaperDetail.level }} - {{ currentPaperDetail.jobType
                                }}</span>
                        </div>
                        <div class="info-item">
        ...
        <div class="modal-overlay" v-if="isModalVisible" @click="closeModal">
            <div class="modal-container" @click.stop>
                <div class="modal-header">
                    <h3>{{ isEditMode ? "编辑" : (isManualMode ? "手动组卷" : "自动组卷") }}</h3>
                    ...
                    <div class="question-type-config">
                        ...
                        <div class="question-type-item" v-for="(type, index) in questionTypes" :key="index">
                            <div class="question-type-header">
                                <span class="question-type-name">{{ type.name }}</span>
                            </div>
                            <div class="question-type-details">
                                <div class="question-type-row">
                                    <div class="question-type-col">
                                        <label>题目数量:</label>
                                        <input type="number" v-model="type.count" min="0" max="100"
                                            class="form-input question-count-input" />
                                    </div>
                                    <div class="question-type-col">
                                        <label>每题分值:</label>
                                        <input type="number" v-model="type.score" min="0" max="100" step="0.5"
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" @click="savePaper" :disabled="isSaving">{{ isSaving ? '保存中...' :
                        '确定'
                        }}</button>
                </div>
            </div>
        ...
        <div class="modal-overlay" v-if="isManualModalVisible" @click="closeManualModal">
            <div class="modal-container" @click.stop>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" @click="saveManualPaper" :disabled="isSaving">{{ isSaving ? '保存中...'
                        : '确定'
                        }}</button>
                </div>
            </div>
            ...
            <div class="modal-container download-modal" @click.stop>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-row">
                        <label>试卷Logo</label>
                        <div class="form-control">
                            <div class="logo-upload-area" @click="triggerLogoInput" @drop="handleLogoDrop"
                                @dragover.prevent @dragenter.prevent :class="{ 'drag-over': isLogoDragOver }">
...
const showManualAddModal = async () => {
    ...
    // 这里可以添加手动组卷的特殊逻辑
    ElMessage.info("手动组卷功能开发中...");
    isManualModalVisible.value = true; // 使用独立的手动组卷模态框
};
...
const savePaper = async () => {
    ...
    try {
        ...
        if (response && response.code === 200) {
            ElMessage.success(isEditMode.value ? "试卷更新成功" : "自动组卷成功");
            closeModal();
            ...
            throw new Error(
                response?.msg || (isEditMode.value ? "更新试卷失败" : "自动组卷失败")
            );
        }
    } catch (error) {
        console.error(isEditMode.value ? "更新试卷失败:" : "自动组卷失败:", error);
        ElMessage.error(
            error.message ||
            (isEditMode.value ? "更新试卷失败，请稍后重试" : "自动组卷失败，请稍后重试")
        );
    } finally {
...
const saveManualPaper = async () => {
    ...
    try {
        // 手动组卷的特殊逻辑
        ElMessage.info("手动组卷功能正在开发中，暂时使用自动组卷逻辑");
        ...
        const response = await constructExam(apiData);
